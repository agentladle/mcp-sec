...
company_tickers.json   ──→       company_tickers.json         (股票代码→CIK 映射)
                                     │
SEC Submissions API    ──→        html/{TICKER_FORM_DATE}/    (工具 2：下载 primary + HTML exhibits)
                                     │
...
| 2 | `download_sec_report` | 下载 SEC 报告；6-K/8-K 默认含 HTML Exhibit（PDF 跳过） |
...
| 7 | `lookup_ticker_cik` | **诊断工具**：查询 ticker→CIK 映射；在 CIK 解析失败时使用 |
...
从 EDGAR 下载指定的 SEC 报告。对 **6-K/8-K** 默认同时下载 **HTML Exhibit**（PDF Exhibit 仅记录为跳过，不解析）。幂等操作。

| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `ticker` | string | ✅ | 股票代码，例如 `"AAPL"` |
| `form` | string | ✅ | 报告类型：`"10-K"`、`"10-Q"`、`"20-F"`、`"6-K"`、`"8-K"` |
| `report_date` | string | ✅ | 报告日期（财政年度截止日期），例如 `"2025-01-31"` |
| `include_exhibits` | bool | ❌ | 是否下载 HTML 附件。默认：6-K/8-K 为 `true`，其余为 `false`。PDF 永不解析 |
...
### 工具 7：`lookup_ticker_cik`

诊断工具：查询 ticker→CIK 映射。仅在 `download_sec_report` / `list_sec_filings` 返回 `CIK not found` 或 `Ticker not found` 时使用。可绕过会话级失败缓存，并返回同 CIK 的别名 ticker。

| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `ticker` | string | ✅ | 股票代码，例如 `"BABA"` |
| `refresh` | bool | ❌ | 是否强制从 SEC 重新下载 `company_tickers.json`，默认 `false` |
...
> ⚠️ **SEC User-Agent 政策**：SEC 要求 User-Agent 头部中包含真实邮箱。使用默认占位邮箱可能导致 IP 被封禁，并引发 ticker→CIK 映射间歇性解析失败。`SEC_EMAIL` 不是可选项，请务必配置。
